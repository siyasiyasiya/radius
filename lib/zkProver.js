...
function packPublicSignals(publicSignals) {
  ...
  // Circuit outputs 4 public signals: [minLat, maxLat, minLon, maxLon]
  // These are the public inputs to the circuit (bounding box)
  if (!Array.isArray(publicSignals) || publicSignals.length < 4) {
    throw new Error(`Unexpected publicSignals from circuit: got ${publicSignals?.length || 0} signals, expected at least 4`);
  }
